    ...
    <form method="post" action="{{ route('requests.store') }}" enctype="multipart/form-data" class="mt-7 space-y-6">
        ...
        <section class="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <div class="mt-5 grid gap-5 md:grid-cols-2">
                ...
                <div><label>Subject & facilitator *</label><select name="subject_id" required><option value="">Select subject</option>@foreach($assignments->groupBy(fn($assignment) => $assignment->subject->year_level) as $yearLevel => $yearAssignments)<optgroup label="{{ $yearLevel ? 'Year '.$yearLevel : 'Other subjects' }}">@foreach($yearAssignments as $assignment)<option value="{{ $assignment->subject_id }}" @selected(old('subject_id')==$assignment->subject_id)>{{ $assignment->subject->code }} — {{ $assignment->subject->name }} ({{ $assignment->faculty->user->name }})</option>@endforeach</optgroup>@endforeach</select>@if($assignments->isEmpty())<p class="mt-2 text-sm text-red-600">No instructors are assigned to your current subjects. Please contact the administrator.</p>@endif</div>
                <div><label>Start time <span class="font-normal text-slate-400">(required when submitting)</span></label><input id="start-time" type="time" name="start_time" value="{{ old('start_time') }}"></div>
                <div><label>End time <span class="font-normal text-slate-400">(required when submitting)</span></label><input id="end-time" type="time" name="end_time" value="{{ old('end_time') }}"></div>
