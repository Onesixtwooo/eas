...
class AuthController extends Controller {
    ...
    public function authenticate(Request $r){$data=$r->validate(['email'=>'required|email','password'=>'required']);$user=User::where('email',$data['email'])->first();if($user && Hash::check($data['password'],$user->password)){if($user->hasRole('student') && $user->registration_declined_at){$reason=$user->registration_decline_reason ?: 'No reason was provided.';return back()->withErrors(['email'=>"Your student registration was declined. Reason: {$reason}"])->onlyInput('email');}if(! $user->is_active){return back()->withErrors(['email'=>'Your account is disabled, please report to the IT office.'])->onlyInput('email');}if($user->hasRole('student') && ! $user->email_verified_at){$r->session()->put('email_verification_user_id',$user->id);return redirect()->route('register.verify-email')->with('error','Verify your email address before signing in.');}if($user->hasRole('student') && ! $user->registration_verified_at){return back()->withErrors(['email'=>'Your registration is awaiting administrator verification. Please try again after your account has been confirmed.'])->onlyInput('email');}}if(Auth::attempt($data+['is_active'=>true],$r->boolean('remember'))){$r->session()->regenerate();$user=Auth::user();$r->session()->put('active_role',$user->getRawOriginal('role'));if($user->isDualRole()){return redirect()->route('access-mode.create');}return redirect()->intended(route('dashboard'));}return back()->withErrors(['email'=>'The credentials do not match our records.'])->onlyInput('email');}
    ...
    public function emailReset(Request $r)
    {
        ...
        return back()->with('success', 'If an account matches that email address, a password reset link has been sent.');
    }
